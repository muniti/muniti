...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #MunitiTEST0\r");
            Send(hSocket, "WHO #MunitiTEST0\r");
        } else {
            // randomly join #Muniti00-#Muniti99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #Muniti%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Muniti%02d\r", channel_number).c_str());
        }
