...
uint256 hashGenesisBlock("0x000000eb3882dcd0ce9c11d085f7c36929f1436d58605b674f8c9ed03ed4d75f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Muniti: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Muniti Signed Message:\n";
...
{
    // Muniti: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 39 * COIN;
	
	if (nHeight == 1)
		return 24334133 * COIN;
	else if (nHeight < 200)
		return 5 * COIN; 
	else if (nHeight < 400)
		return 20 * COIN; 
...
{
    // Muniti: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
		...
		const char* pszTimestamp = "Cryptocurrency of Malta - launched on Freedom Day 2014";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime = 1396036540;
        block.nBits = 0x1e0ffff0;
        block.nNonce = 3974387;

        if (fTestNet)
        {
            block.nTime    = 1396036541;
            block.nNonce   = 0;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xa1a8f2b099c37c67e8ff6ed3bb28e76125da2ebbafd22b81bdc2c2f807faf2ad"));
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc0, 0xb8, 0xde }; // Muniti: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MunitiMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MunitiMiner : ProcessBlock, block not accepted");
    }
